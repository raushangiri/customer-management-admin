  ...
  const handleDeleteClick = async (id) => {
    try {
      if (window.confirm('Are you sure you want to delete this reference?')) {
        setLoading(true);
        await axios.delete(`${baseurl}/deletereferencedetail/${id}`); // Send DELETE request to API
        fetchReferences(); // Fetch updated references list after deletion
      }
    } catch (error) {
      ...
      <div className="position-relative tab-pane active">
        ...
        <div className="mb-4">
          {references && references.length > 0 ? (
              ...
              <tbody>
                {references.map((reference) => (
                  <tr key={reference._id}>
                    ...
                    <td>{reference.company_name}</td>
                    <td>
                      <button className="btn btn-info btn-sm me-2" onClick={() => handleEditClick(reference)}>
                        Edit
                      </button>
                      </td>
                      <td>
                      <button className="btn btn-danger btn-sm" onClick={() => handleDeleteClick(reference._id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
            ...
            <p>No reference details available.</p>
          )}
        </div>
      </div>
    </>
