...
const UploadbankStatement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        await axios.put(`${baseurl}/updateBankStatement/${editId}`, dataToSubmit);
        setEditMode(false);
        setEditId(null);
      } else {
        await axios.post(`${baseurl}/createbankStatement`, dataToSubmit);
  ...
  const handleEdit = (statement) => {
    setEditMode(true);
    setEditId(statement._id);
    setBankAccountNumber(statement.bankAccountNumber);
    setTotalAB(statement.totalAB);
    setSixMonthABB(statement.sixMonthABB);
    setOneYearABB(statement.oneYearABB);
    setPrevious12Months(statement.months);
  };
  ...
  return (
    <div className="container mt-5">
      <h3 className="text-center mb-4">{editMode ? 'Edit Bank Statement' : 'Bank Statement Update Form'}</h3>
      ...
      <h3 className="mt-5 text-center">Submitted Bank Statements</h3>
            ...
            {submittedData.map((statement, index) => (
              <tr key={index}>
               ...
               <button
                  ...
                  <button className="btn btn-info mx-2" onClick={() => handleEdit(statement)}>
