...
//       try {
//         setLoading(true);
//         const response = await axios.get(`${baseurl}/getteamleaderLoanFilesByFilters`, {
//           params: {
//             startDate,
//             endDate,
...
const Teamperformance = () => {
    const [startDate, setStartDate] = useState(new Date().toISOString().split('T')[0]); // Default to current date
    const [endDate, setEndDate] = useState(new Date().toISOString().split('T')[0]); // Default to current date
    const [selectedAgent, setSelectedAgent] = useState('');
    const [teamData, setTeamData] = useState([]);
    const [filteredFiles, setFilteredFiles] = useState([]); // State for filtered results
    const [loading, setLoading] = useState(false);
    ...
    // Fetch data initially with the current date filter on component mount
    useEffect(() => {
        fetchFilteredData();
    }, [startDate, endDate]); // Trigger the fetch when dates change
