...
const Admincdrhistory = () => {
    ...
    const fetchLoanFiles = async (start, end, agent = '', tl = '') => {
      ...
      setError(null);
      try {
        const response = await axios.get(`${baseurl}/getcdrFilesByDate`, {
          params: {
            startDate: start,
            endDate: end,
          },
            ...
            <tbody>
              {loanFiles.map((loanFile, index) => (
                <tr key={loanFile._id}>
                  ...
                  <td>
                    {loanFile.cdr_assign_date && (
                      <>
                        {new Date(loanFile.cdr_assign_date).toLocaleDateString('en-GB')}<br />
                        {new Date(loanFile.cdr_assign_date).toLocaleTimeString('en-US', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </>
